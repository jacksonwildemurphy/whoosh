...
/* This function is run for a group of commands separated
  by the || operator, or the special case of a single command.
  All commands are run at the same time,
...
/* Constructs the argv array for a command */
...
/* Converts a numeric value to a string and installing it as a variable's
   value */
static void set_var(script_var *var, int new_value) {
  char buffer[32];
  free((void *)var->value);
